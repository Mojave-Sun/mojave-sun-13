...
/obj/effect/proc_holder/spell/targeted/shapeshift/cast(list/targets, mob/user = usr)
	...
	for(var/mob/living/shapeshifted_targets in targets)
		if(!shapeshift_type)
			...
			var/new_shapeshift_type = show_radial_menu(shapeshifted_targets, shapeshifted_targets, display_animals, custom_check = CALLBACK(src, PROC_REF(check_menu), user), radius = 38, require_near = TRUE)
			if(shapeshift_type)
				return
