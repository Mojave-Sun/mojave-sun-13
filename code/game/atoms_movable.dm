...
/atom/movable/proc/throw_at(atom/target, range, speed, mob/thrower, spin = TRUE, diagonals_first = FALSE, datum/callback/callback, force = MOVE_FORCE_STRONG, gentle = FALSE, quickstart = TRUE)
	...
	*/
	...
	to_chat(world, "estimated time to target; [time_to_target_s / 10]")
	var/horiz_pixels_to_target = (get_dist(loc, target) < range ? get_dist(thrower, target) : range) * 32
	/*
	...
	*/
	var/gravity_deacceleration = -8
	var/new_vertical_velocity = ((-8 / time_to_target_s) - (0.5(-8 * time_to_target_s)))
	new_vertical_velocity *= (rand(90, 110) / 100)
	...
	AddComponent(/datum/component/movable_physics, _horizontal_velocity = rand(5, 6), _vertical_velocity = new_vertical_velocity, _horizontal_friction = 0.20, _z_gravity = 8, _z_floor = 0, _angle_of_movement = get_angle(get_turf(src), target) + rand(-15, 15))
	/*
