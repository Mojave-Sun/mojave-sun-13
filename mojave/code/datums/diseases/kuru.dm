	...
	switch(stage)
		if(1)
			...
			if(DT_PROB(3, delta_time))
				affected_mob.Jitter(20 SECONDS)
		if(2)
			if(DT_PROB(1, delta_time))
				affected_mob.Knockdown(20 SECONDS)
		...
		if(3)
			...
			if(DT_PROB(2, delta_time))
				affected_mob.add_confusion(10 SECONDS)
		...
		if(4)
			...
			if(DT_PROB(3, delta_time))
				affected_mob.emote("scream")
			if(DT_PROB(3, delta_time))
				affected_mob.add_confusion(20 SECONDS)
			if(DT_PROB(7.5, delta_time))
				affected_mob.Jitter(20 SECONDS)
			if(DT_PROB(10, delta_time))
				affected_mob.stuttering += 5
